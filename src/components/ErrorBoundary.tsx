    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#0B1120] p-4">
          <div className="max-w-md w-full bg-white dark:bg-[#1A1D23] rounded-3xl p-8 shadow-xl border border-slate-200 dark:border-white/10 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Application Error</h2>
              <p className="text-slate-600 dark:text-slate-400 font-medium">{errorMessage}</p>
              {details && <p className="text-xs text-red-500 font-mono mt-2 break-all">{details}</p>}
            </div>
            <button
