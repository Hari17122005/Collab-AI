...
export function Sidebar({ currentPath, onNavigate }: SidebarProps) {
  ...
  return (
    <aside 
      className={cn(
        "flex flex-col py-6 bg-white dark:bg-[#0F1115] border-r border-slate-200 dark:border-white/5 h-screen sticky top-0 z-30 transition-all duration-500 ease-in-out group/sidebar",
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide px-2 space-y-10">
        ...
        <div className="flex flex-col gap-2 w-full">
          <p className={cn(
            "text-[10px] font-bold text-slate-400 dark:text-slate-600 tracking-widest mb-4 uppercase transition-all",
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <span className="text-sm font-bold flex-1 text-left animate-in fade-in slide-in-from-left-2 duration-300">
                    {item.label}
                  </span>
                )}
                {item.badge && isExpanded && (
                  <div className="h-5 w-5 bg-amber-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-lg shadow-amber-500/20">
                    {item.badge}
                  </div>
                )}
                {isActive && !isExpanded && (
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); onNavigate('/apps'); }}
            className={cn(
              "flex items-center transition-all duration-300 group cursor-pointer",
              isExpanded ? "px-4 py-2 gap-4" : "justify-center"
            )}
      ...
      <div className={cn(
        ...
        isExpanded ? "flex items-center justify-between bg-slate-100 dark:bg-white/5 p-2 rounded-2xl mx-4" : "flex flex-col items-center gap-2 bg-slate-100 dark:bg-white/5 p-1.5 rounded-2xl mx-2"
      )}>
        <button 
          onClick={(e) => { e.stopPropagation(); theme === 'dark' && toggleTheme(); }}
          className={cn(
            "p-2 rounded-xl transition-all",
            theme === 'light' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-300"
          )}
        >
          <Sun className="h-5 w-5" />
        </button>
        <button 
          onClick={(e) => { e.stopPropagation(); theme === 'light' && toggleTheme(); }}
          className={cn(
            "p-2 rounded-xl transition-all",
            theme === 'dark' ? "bg-[#1A1D23] text-blue-400 shadow-sm" : "text-slate-500 hover:text-slate-700"
          )}
        >
