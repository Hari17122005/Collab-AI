...
export function AuthProvider({ children }: { children: React.ReactNode }) {
          ...
          unsubscribeProfile = onSnapshot(userRef, async (userSnap) => {
            try {
              ...
              if (userSnap.exists()) {
                ...
                if (selectedRole && !profile.teamId && profile.role !== selectedRole) {
                  await updateDoc(userRef, { role: selectedRole });
                  // The next snapshot will have the updated role
                  return;
                }
              } else {
                const selectedRole = sessionStorage.getItem('selectedRole') || 'Team Member';
                profile = {
                  uid: user.uid,
  ...
  return (
    <AuthContext.Provider value={{ currentUser, userProfile, teamData, loading, logout }}>
      {!loading && children}
