...
import { MessageSquare, Clock, AlertCircle, MoreVertical, Filter, Plus, CheckCircle2 as CheckIcon, Pause, LayoutDashboard, ChevronDown } from 'lucide-react';
...
export function Tasks() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-full text-slate-500 dark:text-slate-400">Loading tasks...</div>;
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500 h-full flex flex-col">
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
          <span>Projects MT</span>
          <span>/</span>
          <span>Commercial Projects</span>
          <span>/</span>
          <span className="text-slate-600 dark:text-slate-300">Team Phoenix</span>
        </div>
        ...
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">All team members: <span className="text-blue-600 dark:text-blue-400">12</span></span>
                <div className="flex -space-x-3">
                  {users.slice(0, 5).map((user, i) => (
                    <Avatar key={i} fallback={user.name.charAt(0)} size="sm" className="border-2 border-white dark:border-[#0A0B0D]" />
                  ))}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-white dark:bg-white/5 p-1 rounded-2xl border border-slate-200 dark:border-white/5">
              ...
              <div key={col.id} className="w-[400px] flex flex-col gap-6">
                <div className="flex items-center justify-between px-2">
                  <div className="flex items-center gap-4">
                    <Badge className={cn("px-4 py-1.5 rounded-full text-white font-bold border-none", col.color)}>
                      {col.label}
                    </Badge>
                ...
                <div className="flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
                  {colTasks.map(task => (
                    <Card 
                      key={task.id} 
                      className="group cursor-pointer border-transparent hover:border-blue-500/30 dark:hover:border-blue-500/20 transition-all duration-300"
                      onClick={() => handleTaskClick(task)}
                    >
                      <CardContent className="p-6 space-y-6">
                        <div className="flex items-center justify-between">
                          <div className="flex gap-2">
                            <Badge variant="secondary" className="bg-purple-100 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border-none px-3 py-1">
                              Task list
                            </Badge>
                            <Badge variant="secondary" className="bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-none px-3 py-1">
                              Personal
                            </Badge>
                          </div>
                          <button 
                            onClick={(e) => { e.stopPropagation(); /* Add task logic */ }}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-100 dark:border-white/5">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3">Responsible</p>
                            <div className="flex items-center gap-2">
                              <Avatar fallback={users.find(u => u.id === task.assignees[0])?.name.charAt(0) || '?'} size="sm" />
                              <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 uppercase truncate">
                                {users.find(u => u.id === task.assignees[0])?.name || 'Unassigned'}
                              </span>
                            </div>
                          </div>
                          <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-100 dark:border-white/5 relative overflow-hidden">
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Tasks</p>
                          ...
                          {[
                            ...
                            <div key={i} className="flex items-center gap-3">
                              <div className={cn(
                                ...
                                item.checked ? "bg-slate-900 dark:bg-white border-slate-900 dark:border-white" : "border-slate-300 dark:border-white/20"
                              )}>
                                {item.checked && <CheckIcon className="h-3 w-3 text-white dark:text-slate-900" />}
                              </div>
                              <span className={cn(
                                "text-sm font-medium transition-colors",
                                item.checked ? "text-slate-400 line-through" : "text-slate-600 dark:text-slate-300"
                              )}>
                                {item.label}
                              </span>
                            </div>
                  ...
                  {colTasks.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 border-2 border-dashed border-slate-200 dark:border-white/5 rounded-3xl">
                      ...
                      <p className="text-sm font-bold text-slate-400">No tasks in this stage</p>
                    </div>
                  )}
                </div>
              </div>
