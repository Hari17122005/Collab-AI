...
import { Target, TrendingUp, Award, Clock, Plus, Loader2, Trash2 } from 'lucide-react';
...
export default function Goals() {
  ...
  const [newGoal, setNewGoal] = useState({ title: '', category: 'Product', progress: 0, deadline: '' });
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">Loading Goals...</p>
      </div>
    );
  }
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-wrap items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Strategic Goals</h1>
          <p className="text-slate-500 dark:text-slate-400">Track and manage your long-term objectives</p>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="bg-white dark:bg-white/5 px-6 py-3 rounded-2xl border border-slate-200 dark:border-white/5 flex items-center gap-4">
            <div className="text-center">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Completed</p>
              <p className="text-xl font-bold text-emerald-500">{goals.filter(g => g.progress === 100).length}</p>
            </div>
            <div className="w-px h-8 bg-slate-200 dark:bg-white/10" />
            <div className="text-center">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">In Progress</p>
              <p className="text-xl font-bold text-blue-500">{goals.filter(g => g.progress < 100).length}</p>
            </div>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
              ...
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Goal Title</label>
                    <input 
                      type="text"
          ...
          {goals.map((goal) => (
            <Card key={goal.id} className="border-transparent bg-white/50 dark:bg-white/5 backdrop-blur-xl hover:border-blue-500/30 transition-all group">
              <CardContent className="p-8">
                <div className="flex flex-wrap items-start justify-between gap-6 mb-8">
                  <div className="flex gap-5">
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-1">{goal.title}</h3>
                        <button 
                  ...
                  <div className="text-right flex flex-col items-end gap-2">
                    <div className="text-3xl font-bold text-slate-900 dark:text-white">{goal.progress}%</div>
                ...
                {goal.subgoals?.length > 0 && (
                  <div className="space-y-4">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sub-goals</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {goal.subgoals.map((sub: any, j: number) => (
                        <div key={j} className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-bold text-slate-700 dark:text-slate-300">{sub.title}</span>
                            <span className="text-xs font-bold text-blue-600">{sub.progress}%</span>
                          </div>
        ...
        <div className="space-y-8">
          <Card className="border-transparent bg-gradient-to-br from-blue-600 to-blue-700 text-white">
            <CardContent className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-blue-100 text-sm leading-relaxed">Focus on high-impact goals to maintain your team's momentum.</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-transparent">
            ...
            <CardContent>
              <p className="text-sm text-slate-500 leading-relaxed italic">
                Manage your long-term vision. Add goals and track their progress in real-time with your team.
              </p>
            </CardContent>
          </Card>
        </div>
