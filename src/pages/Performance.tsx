...
export function Performance() {
  ...
  if (!userProfile) {
    return <div className="flex items-center justify-center h-full text-slate-500 dark:text-slate-400">Loading performance data...</div>;
  }

  const radarData = [
    { subject: 'Completion', A: userProfile.tasksCompleted || 0, fullMark: 100 },
    ...
    { subject: 'Productivity', A: userProfile.productivityScore || 0, fullMark: 100 },
  ];

  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      {/* Profile Header */}
      <Card className="border-transparent bg-blue-600/5 overflow-hidden relative">
        ...
        <CardContent className="p-10 flex flex-col md:flex-row items-center gap-10 relative z-10">
          <div className="relative">
            <Avatar fallback={userProfile.name?.charAt(0) || '?'} size="lg" className="h-32 w-32 border-4 border-white dark:border-[#1A1D23] shadow-2xl shadow-blue-500/20" />
          ...
          <div className="flex-1 text-center md:text-left space-y-2">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{userProfile.name}</h2>
          ...
          <div className="flex gap-10 bg-white/50 dark:bg-white/5 backdrop-blur-md p-6 rounded-3xl border border-white dark:border-white/5">
            <div className="text-center">
              <div className="text-4xl font-bold text-emerald-500">{userProfile.healthScore || 0}</div>
              <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Health Score</div>
            </div>
            <div className="w-px bg-slate-200 dark:bg-white/10" />
            <div className="text-center">
              <div className="text-4xl font-bold text-blue-500">{userProfile.productivityScore || 0}</div>
              <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Productivity</div>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="space-y-8 md:col-span-2">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {[
              { label: 'Tasks Completed', value: userProfile.tasksCompleted || 0, icon: Target, color: 'text-purple-500', bg: 'bg-purple-500/10' },
              { label: 'Punctuality Rate', value: `${userProfile.punctuality || 0}%`, icon: Clock, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
              { label: 'Engagement Score', value: `${userProfile.engagement || 0}%`, icon: Zap, color: 'text-amber-500', bg: 'bg-amber-500/10' },
              { label: 'Current Streak', value: '14 Days', icon: Award, color: 'text-blue-500', bg: 'bg-blue-500/10' },
            ].map((stat, i) => (
              <Card key={i} className="border-transparent hover:border-blue-500/30 transition-all group">
            ...
            <CardContent className="space-y-4">
              {userProfile.tasksCompleted && userProfile.tasksCompleted > 10 ? (
                <div className="flex items-center gap-5 p-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 hover:border-blue-500/30 transition-all group">
                  ...
                  <div>
                    <p className="text-base font-bold text-slate-900 dark:text-white">Task Master</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Completed over 10 tasks successfully.</p>
                  </div>
                </div>
              ) : (
        ...
        <Card className="border-transparent">
          <CardHeader>
            <CardTitle className="text-xl font-bold">Skill Analysis</CardTitle>
          </CardHeader>
