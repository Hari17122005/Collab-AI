...
import { Task, User, JoinRequest, App, SecurityLog, TeamSettings, Favorite } from '../types';
...
export function useData() {
  ...
  useEffect(() => {
    ...
    const unsubscribeLogs = onSnapshot(qLogs, (snapshot) => {
      ...
      setSecurityLogs(logsData.sort((a, b) => {
    ...
    const unsubscribeSettings = onSnapshot(doc(db, 'teamSettings', userProfile.teamId), (snapshot) => {
      if (snapshot.exists()) {
        setTeamSettings({ id: snapshot.id, ...snapshot.data() } as TeamSettings);
      } else {
        setTeamSettings({
      ...
      handleFirestoreError(error, OperationType.GET, `teamSettings/${userProfile.teamId}`);
    });
    ...
    const unsubscribeRoadmap = onSnapshot(qRoadmap, (snapshot) => {
  ...
  return { tasks, users, joinRequests, apps, securityLogs, teamSettings, favorites, roadmapPhases, goals, loading };
}
